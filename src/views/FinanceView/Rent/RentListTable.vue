  ...
  <v-jumbotron height="auto">
    <v-container grid-list-xl fill-height>
      <v-layout align-start align-content-start justify-center wrap>
        <v-flex xs12 xl10>
          <v-subheader>待缴租金账单</v-subheader>
          <v-card>
            <v-toolbar flat dense color="white" style="overflow-x: auto;">
              ...
              <v-btn :disabled="!searchHouseValid" @click="initialize" large flat color="primary" class="ma-0">开始搜索</v-btn>
            </v-toolbar>
            <v-divider></v-divider>
            <v-data-table
            ...
            >
              ...
              <template slot="items" slot-scope="props">
                <td v-for="row in headers" :key="row.value" v-if="row.value">{{ row.getData(props.item[row.value]) }}</td>
