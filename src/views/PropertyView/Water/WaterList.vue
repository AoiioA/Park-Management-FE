    ...
    <view-tool-bar>
      ...
      <span slot="bar-extend">
        <v-tabs color="primary" class="px-4" show-arrows>
          <v-tab v-for="tab in viewToolBarTab" :key="tab.value" :to="{ name: 'water-list', params: { waterType: tab.value } }">{{ tab.name }}</v-tab>
        </v-tabs>
      </span>
    </view-tool-bar>
          ...
          <v-flex xs12 xl10>
            <v-card>
              <v-toolbar flat dense color="white" style="overflow-x: auto;">
                ...
                <v-divider vertical></v-divider>
                <v-dialog ref="monthdialog" :return-value.sync="searchFilter.waterMonth" lazy full-width width="290px" style="min-width: 132px">
                  <v-text-field slot="activator" v-model="searchFilter.waterMonth" label="月份" flat solo hide-details clearable readonly></v-text-field>
...
export default {
  ...
  data: () => ({
    viewToolBarTab: [
      {
        name: "待缴账单",
        value: "paid",
        url: "/cms/waterBillSub/selectWaterBillSub.json"
      },
      {
        name: "全部账单",
        value: "all",
        url: "/cms/waterBillSub/queryWaterBillSub.json"
      }
    ],
    ...
    headers: [
      ...
      { text: "用水量", value: "totalAmount", getData: v => v, align: "right" },
      {
        text: "账单总额",
        value: "totalWaterFee",
        getData: v => v,
  ...
  methods: {
    initialize() {
      ...
      this.$http
        .post(
          this.viewToolBarTab.find(item => item.value == this.waterType).url,
